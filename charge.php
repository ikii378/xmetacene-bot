...
$body = json_encode([
    'user' => [
        'recharge' => [
            'spar' => 50000000 // Set the spar value to 500000
        ]
    ]
]);
...
foreach ($cookies as $item) {
    ...
    if (curl_errno($ch)) {
        ...
        if (isset($data['data']['spar'])) {
            $sparValue = $data['data']['spar'];
            $pesan = "Berhasil !!! Total Charge ($alias): " . $sparValue;
            echo $pesan;
            kirimKeTelegram($pesan);
        } else {
            // Jika data recharge -> spar tidak tersedia
            $pesan = "Gagal !!! Total Charge ($alias): " . $sparValue;
            echo $pesan;
            kirimKeTelegram($pesan);
        }
    }
